...
final public class Ship extends MarineVehicle implements Serializable {
    public Ship(City city, int orderNumber, ShippingPort shippingPort, int ID) throws IOException, NotEnoughMoneyException, NoRoomException {
        super(ID, orderNumber);
        ArrayList<String> properties = Country.sellListReader("Ship.csv", orderNumber);
        ...
        if (shippingPort.getVehicles().size() >= shippingPort.getMaxVehicle())
            throw new NoRoomException("There is no room for adding new cargo ship");
        isFirstMarineVehicle = false;
        super.ID += ++numberOFCreatedVehicle;
    }
    public Ship(City city, int orderNumber, ShippingPort shippingPort) throws IOException, NotEnoughMoneyException, NoRoomException {
