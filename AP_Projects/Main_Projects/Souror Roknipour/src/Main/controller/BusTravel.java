...
public class BusTravel implements Initializable {
    @FXML
    private TableView<Person> LView;
    @FXML
    private TableView<Inter_city_bus> BusView;
    @FXML
    private TableView<Bus_Terminal> desTView;
    @FXML
    private TableView<Bus_Terminal> OriTView;
    @FXML
    ...
    private Bus_Terminal terminal;
    private Bus_Terminal DesRailway;
    private Inter_city_bus Bus;
    ...
    void DesCityBTN(ActionEvent event) {
        if (!CityView.getSelectionModel().isEmpty()) {
            ...
            for (Terminal x : desCity.getTerminals()) {
                if (x instanceof Bus_Terminal) desTView.getItems().add((Bus_Terminal) x);
            }
    ...
    void DoneBTN(ActionEvent event) {
        if (passengers != null && driver != null && DesRailway != null && terminal != null && Bus != null && desCity != null) {
            if (checkCapacity(1 + passengers.size(), Bus) || checkFullCapacity(1 + passengers.size(), Bus)) {
                JOptionPane.showMessageDialog(null, "Error in Capacity!", "Error", JOptionPane.ERROR_MESSAGE);
            } else {
                terminal.travel(DesRailway, Bus, driver, passengers);
                passengers.clear();
                driver = null;
                Bus = null;
                DesRailway = null;
    ...
    void vehicleBTN(ActionEvent event) {
        if (!BusView.getSelectionModel().isEmpty()) {
            Bus = BusView.getSelectionModel().getSelectedItem();
            JOptionPane.showMessageDialog(null, "Done ! ", "successful", JOptionPane.INFORMATION_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(null, "Fields are not completed!", "Error", JOptionPane.ERROR_MESSAGE);
        }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        //--------------------------------------------------------
        FXMLLoader loader2 = new FXMLLoader(this.getClass().getResource("../view/tableShow/BusTerminals.fxml"));
        try {
            loader2.load();
        } catch (IOException ioException) {
            ioException.printStackTrace();
        ...
        //----------------------------------------------------------
        FXMLLoader loader5 = new FXMLLoader(this.getClass().getResource("../view/tableShow/Buses.fxml"));
        try {
            loader5.load();
        } catch (IOException ioException) {
            ioException.printStackTrace();
